...
// ✅ SMART CSV PROCESSOR - FIXED FIREBASE IMPLEMENTATION
class SmartCSVProcessor {
        ...
        try {
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            if (isValidContact) {
              ...
              const contact = {
                business: row.business_name || row.business || 'Unknown Business',
                email: email || null,
                phone: phone || null,
                website: row.website || '',
                industry: row.industry || 'SaaS',
                size: row.company_size || row.employees || 'Unknown',
  ...
  // ✅ FIXED: Save contacts individually to avoid serverTimestamp in arrays
  async saveContactsToFirestore(contacts, userId) {
    ...
    for (const contact of contacts) {
      try {
        ...
        console.error('Failed to save contact:', contact.business, error);
      }
    }
  }
}
...
class SmartLeadDiscovery {
  ...
  async discoverLeads(source, filters, userId) {
    // Simulate lead discovery with realistic data
  ...
  generateMockLeads(source, filters) {
    ...
    for (let i = 0; i < count; i++) {
      const lead = {
        ...
        business: `TechSaaS ${i + 1}`,
        email: `contact${i + 1}@techsaas${i + 1}.com`,
        phone: `+1${Math.floor(Math.random() * 9000000000) + 1000000000}`,
        website: `https://techsaas${i + 1}.com`,
        industry: filters.industry || 'SaaS',
        size: filters.size || '50-100 employees',
        funding: 'Series A',
        location: filters.geography?.includes('North America') ? 'San Francisco, CA' : 'London, UK',
        description: `Leading ${filters.industry || 'SaaS'} company helping businesses scale`,
        source,
        discoveredAt: new Date(),
        quality: Math.random() > 0.7 ? 'hot' : Math.random() > 0.4 ? 'warm' : 'cold',
        leadScore: Math.floor(Math.random() * 40) + 60,
        decisionMaker: `John Doe ${i + 1}`,
        decisionMakerTitle: 'CEO' || 'CTO' || 'VP Engineering',
  ...
  // ✅ FIXED: Save discovered leads individually
  async saveDiscoveredLeads(leads, userId) {
    const contactsRef = collection(db, 'users', userId, 'contacts');
      ...
      try {
        ...
        await setDoc(docRef, {
          ...
          statusHistory: [{
            status: 'new',
            timestamp: serverTimestamp(),
            note: `Discovered via ${LEAD_SOURCES[lead.source].name}`
          }]
        });
      } catch (error) {
        console.error('Failed to save discovered lead:', lead.business, error);
      }
    }
  }
}

export default function CompleteSalesAutomation() {
  ...
  const [discoveryStatus, setDiscoveryStatus] = useState({ discovering: false, source: null, results: [] });
  ...
  const [followUpTemplates, setFollowUpTemplates] = useState([
    { id: 1, name: 'Follow-Up 1', subject: FOLLOW_UP_1.subject, body: FOLLOW_UP_1.body, enabled: true, delayDays: 3 },
    { id: 2, name: 'Follow-Up 2', subject: FOLLOW_UP_2.subject, body: FOLLOW_UP_2.body, enabled: true, delayDays: 5 },
    { id: 3, name: 'Break-up Email', subject: FOLLOW_UP_3.subject, body: FOLLOW_UP_3.body, enabled: true, delayDays: 7 }
  ]);
  ...
  const loadContacts = useCallback(async () => {
    ...
    try {
      ...
      setCampaign(prev => ({
        ...prev,
        totalLeads: loadedContacts.length
      }));
  ...
  const handleCsvUpload = useCallback(async (e) => {
    ...
    try {
      ...
      if (user?.uid && result.contacts.length > 0) {
        addNotification('info', `Saving ${result.contacts.length} contacts to database...`);
        await csvProcessor.current.saveContactsToFirestore(result.contacts, user.uid);
      }
      ...
      addNotification('success', `Successfully imported ${result.contacts.length} contacts`);
  ...
  const handleLeadDiscovery = useCallback(async (source) => {
    setDiscoveryStatus(prev => ({ ...prev, discovering: true, source }));
    
    try {
      addNotification('info', `Discovering leads from ${LEAD_SOURCES[source].name}...`);
      
      const result = await leadDiscovery.current.discoverLeads(source, {
        industry: 'SaaS',
        size: '20-200',
      ...
      if (user?.uid && result.leads.length > 0) {
        await leadDiscovery.current.saveDiscoveredLeads(result.leads, user.uid);
      }
      ...
      addNotification('success', `Discovered ${result.leads.length} new leads from ${LEAD_SOURCES[source].name}`);
  ...
  const updateContactStatus = useCallback(async (contactId, newStatus) => {
    if (!user?.uid || !contactId || !newStatus) return;
    ...
    try {
      ...
      if (!snapshot.empty) {
        ...
        await updateDoc(contactDoc.ref, {
          status: newStatus,
          lastUpdated: serverTimestamp(),
          statusHistory: [...(contactData.statusHistory || []), {
            status: newStatus,
            timestamp: serverTimestamp(),
  ...
  const startCampaign = useCallback(async () => {
    ...
    const eligibleContacts = contacts.filter(c => c.status === 'new' && c.email);
    if (eligibleContacts.length === 0) {
      addNotification('warning', 'No eligible contacts for campaign');
      return;
    }
    
    setCampaign(prev => ({ ...prev, status: 'running', startDate: new Date() }));
    addNotification('info', `Starting campaign to ${eligibleContacts.length} contacts...`);
    
    // Simulate campaign execution
    let sentCount = 0;
    for (const contact of eligibleContacts.slice(0, 50)) { // Limit to 50 per day
      try {
    ...
    addNotification('success', `Campaign completed. Sent ${sentCount} emails.`);
  }, [contacts, updateContactStatus, addNotification]);
        ...
        <div className="text-center max-w-md">
          ...
          <div className="bg-gray-800 rounded-lg p-6 mb-6">
            ...
            <div className="text-left text-gray-300 space-y-2 text-sm">
              ...
              <p><strong>Size:</strong> {ICP_DEFINITION.size}</p>
              <p><strong>Geography:</strong> {ICP_DEFINITION.geography}</p>
              <p><strong>Pain:</strong> {ICP_DEFINITION.pain}</p>
  ...
  return (
    <>
      <Head>
        <title>Syndicate Solutions - Complete Sales Automation</title>
      </Head>
          ...
          <div className="container mx-auto px-4 py-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-gray-400 text-sm">Complete B2B Sales Automation</p>
              </div>
        ...
        <main className="container mx-auto px-4 py-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
              ...
              <p className="text-white text-3xl font-bold">{campaign.totalLeads}</p>
              <p className="text-gray-500 text-xs mt-1">In database</p>
            </div>
            
            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
              <h3 className="text-gray-400 text-sm font-medium mb-2">Emails Sent</h3>
              <p className="text-blue-400 text-3xl font-bold">{campaign.emailsSent}</p>
              <p className="text-gray-500 text-xs mt-1">Campaign emails</p>
            </div>
            
            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
              <h3 className="text-gray-400 text-sm font-medium mb-2">Replies</h3>
              <p className="text-green-400 text-3xl font-bold">{campaign.replies}</p>
              <p className="text-gray-500 text-xs mt-1">Response rate</p>
            </div>
            
            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
              <h3 className="text-gray-400 text-sm font-medium mb-2">Meetings</h3>
              <p className="text-purple-400 text-3xl font-bold">{campaign.meetingsBooked}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
              ...
              <div className="space-y-3">
                {Object.entries(LEAD_SOURCES).map(([key, source]) => (
                  <button
                    key={key}
                    onClick={() => handleLeadDiscovery(key)}
                    disabled={discoveryStatus.discovering || !source.enabled}
                ...
                <div className="bg-blue-900/30 border border-blue-800 rounded-lg p-3 mt-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-blue-300 text-sm">
                      Discovering leads from {LEAD_SOURCES[discoveryStatus.source]?.name}...
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
              ...
              <div className="space-y-4">
                ...
                >
                  {campaign.status === 'running' ? '📤 Campaign Running...' : '🚀 Start Campaign'}
                </button>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-700">
                  {getFilteredContacts().slice(0, 20).map((contact) => (
