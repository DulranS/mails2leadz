...
import { getFirestore, doc, setDoc, getDoc, collection, query, where, getDocs, updateDoc, deleteDoc, orderBy, limit, addDoc, serverTimestamp, writeBatch } from 'firebase/firestore';
...
// ✅ TIGHT ICP DEFINITION - Business Focused
...
// ✅ CONTROLLED TEMPLATES - 3 Only, <120 words each
const TEMPLATES = {
  email1: {
...
class LeadQualificationEngine {
  static qualifyLead(contact) {
    ...
    // Industry match
    if (contact.industry?.toLowerCase().includes('saas') || 
    ...
    // Funding stage match
    if (contact.funding_stage?.includes('Series') || 
        contact.recent_funding ||
        contact.funding_amount) {
      score += 20;
    ...
    // Geography match
    if (contact.country && 
        ['United States', 'Canada', 'UK', 'Germany', 'France'].includes(contact.country)) {
      score += 15;
    ...
    if (contact.recent_hiring || contact.product_launch || 
        contact.expansion || contact.news_mention) {
      score += 5;
  ...
  static findSimilarCompany(contact) {
    ...
    const examples = ['TechCorp', 'DataFlow', 'CloudBase', 'SaaSPro'];
...
class CampaignManager {
  ...
  async executeSend(contact, email) {
    ...
    console.log(`Sending email to ${contact.email}:`, email);
...
export default function BusinessSalesAutomation() {
  // Core state
  ...
  const handleCSVUpload = async (event) => {
    ...
    setLoading(true);
    addNotification('info', 'Processing CSV with business qualification...');
    ...
    try {
      ...
      const disqualifiedCount = { industry: 0, size: 0, email: 0 };
  ...
  const saveContactsBatch = async (contacts, userId) => {
    ...
    for (let i = 0; i < batches; i++) {
      ...
      try {
        await batch.commit();
        console.log(`Saved batch ${i + 1}/${batches}`);
      } catch (error) {
  ...
  // Execute campaign with business logic
  const executeCampaign = async () => {
    ...
    try {
      ...
      addNotification('success', 
        `Campaign completed: ${sentCount} emails sent, ${errorCount} errors`
      );
  ...
  const kpiMetrics = useMemo(() => {
    ...
    return {
      ...
      pipelineValue: demos * 5000 + won * 10000 // Estimated value
    };
  }, [contacts]);
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center max-w-2xl">
          <div className="mb-8">
            ...
            <p className="text-gray-300 text-xl mb-2">B2B Sales Automation Platform</p>
            <p className="text-gray-500 text-base">Strategic Outreach for Scaling SaaS Companies</p>
          </div>
  ...
  return (
    <>
      <Head>
        <title>Syndicate Solutions - B2B Sales Automation</title>
        ...
        <main className="container mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-6 mb-8">
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <p className="text-white text-3xl font-bold">{kpiMetrics.total}</p>
            ...
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <p className="text-green-400 text-3xl font-bold">{kpiMetrics.qualified}</p>
            ...
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <p className="text-purple-400 text-3xl font-bold">{kpiMetrics.contacted}</p>
            ...
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <p className="text-yellow-400 text-3xl font-bold">{kpiMetrics.replyRate}%</p>
            ...
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <p className="text-orange-400 text-3xl font-bold">{kpiMetrics.demos}</p>
            ...
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              ...
              <p className="text-emerald-400 text-3xl font-bold">
                ${(kpiMetrics.pipelineValue / 1000).toFixed(1)}K
              </p>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-gray-300 text-sm font-medium mb-2">
                    Import CSV with ICP qualification
                  </label>
                  <input
                    type="file"
                    accept=".csv"
                ...
                <div className="bg-blue-900/30 border border-blue-800 rounded-lg p-3">
                  <p className="text-blue-300 text-sm font-medium">🎯 Automatic Qualification</p>
                ...
                {loading && (
                  <div className="bg-amber-900/30 border border-amber-800 rounded-lg p-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-amber-300 text-sm">Processing with business logic...</span>
                    </div>
                  </div>
                )}
              </div>
